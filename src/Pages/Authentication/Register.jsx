...
const Register = () => {
  ...
  const handelGoogle = () => {
    socialAuth(googleProvider).then(() => {
      navigate('/');
      toast.success('Register Successfully.');
    });
  };
    ...
    <>
      <section className="w-11/12 md:w-10/12 mx-auto h-auto flex flex-col-reverse lg:flex-row my-0 lg:my-12">
        ...
        <div className="shadow-md backdrop-blur-3xl rounded-lg sm:py-6 sm:px-8 p-4 flex flex-col gap-5 flex-1">
            ...
            <div>
              ...
              <input
                ...
                {...register('photoUrl', {
                  ...
                  pattern: { value: new RegExp('^https?:\\/\\/.+\\.(png|jpg|jpeg|bmp|gif|webp)$', 'i'), message: 'Invalid URL (png, jpg, jpeg, bmp, gif, webp).' },
                })}
              />
              {errors.photoUrl && (
                <p className="flex text-red-500 gap-1 items-center">
                  <MdError /> {errors.photoUrl.message}{' '}
                </p>
              )}
            </div>
          ...
          <button onClick={handelGoogle} className="flex items-center justify-center py-2 px-4 gap-4 border border-gray-300 rounded-lg w-full text-[1rem] font-medium">
